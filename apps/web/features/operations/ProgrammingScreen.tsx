...
export function ProgrammingScreen({ siteId, queue }: { siteId: string; queue: QueueEntryView[] }) {
  ...
  // A truck whose window expired is already back in the queue, so it is no
  // longer part of this run. It is named once, not left as a dead row.
  const liveItems = activeBatch?.items.filter((item) => item.availabilityStatus !== "EXPIRED") ?? [];
  const expiredItems = activeBatch?.items.filter((item) => item.availabilityStatus === "EXPIRED") ?? [];
  const allOrdersSelected = confirmed.length > 0 && confirmed.every((item) => orderByCycle[item.queueCycleId]);
  const activeStep = activeBatch ? (liveItems.length > 0 && confirmed.length === liveItems.length ? 2 : 1) : 0;
  ...
  // Pair each confirmed truck with the next available order in FIFO order. The
  // officer can change any row, and nothing is recorded until they confirm.
  const autoAssignOrders = () => {
    const taken = new Set<string>();
    const next: Record<string, string> = {};
    for (const item of confirmed) {
      const order = orders.find((option) => !taken.has(option.orderId));
      if (!order) break;
      taken.add(order.orderId);
      next[item.queueCycleId] = order.orderId;
    }
    setOrderByCycle(next);
    setError(Object.keys(next).length < confirmed.length ? "There are fewer imported orders than confirmed trucks. Assign the rest once more orders are imported." : "");
  };
  ...
  return <>
    ...
    <div className="programmingLayout programmingWorkbench">
      ...
      <section className="previewPanel programmingPreviewPanel"><div className="programmingPanelHeading"><div><span>{activeBatch ? "Availability window" : preview ? "FIFO batch preview" : "Next in line"}</span><h2>{activeBatch ? `${confirmed.length} of ${liveItems.length} drivers confirmed` : preview ? `${preview.items.length} FIFO trucks selected` : "The queue is ready when you are"}</h2><p>{activeBatch ? "Confirmed trucks can now receive imported orders and ATCs." : preview ? "Review the suggested order before requesting availability." : "Start with a batch size to see the next eligible trucks."}</p></div>{activeBatch ? <button className="iconButton" disabled={busy !== null} onClick={() => void refreshAvailability()} title="Refresh availability" type="button">{busy === "refresh" ? <LoaderCircle className="spin" size={16} /> : <RefreshCw size={16} />}</button> : <span className="programmingPanelStatus"><span /> Live queue</span>}</div>
        ...
        {activeBatch ? <><div className="availabilitySummary"><Clock3 size={18} /><p>{liveItems.length ? `${confirmed.length} of ${liveItems.length} trucks have confirmed. A truck that misses its hour returns to the back of the queue and the next eligible FIFO truck is asked in its place.` : "No truck confirmed availability in this run. Every truck is back in the queue, so start a new batch when you are ready."}</p></div>
        {expiredItems.length ? <p className="availabilityExpiredNote">{expiredItems.length} truck{expiredItems.length === 1 ? "" : "s"} did not confirm in time and returned to the queue: {expiredItems.map((item) => item.registrationNumber ?? queueByTruck.get(item.truckId)?.registrationNumber ?? item.truckId).join(", ")}.</p> : null}{liveItems.length ? <div className="programmingRoster availabilityRoster"><div aria-hidden="true" className="programmingRosterHeader"><span>Order</span><span>Truck</span><span>Availability</span><span>Imported order and ATC</span></div>{liveItems.map((item) => { const truck = queueByTruck.get(item.truckId); const registration = item.registrationNumber ?? truck?.registrationNumber ?? item.truckId; const driverName = item.driverName ?? truck?.driverName ?? "Driver not recorded"; const eligible = item.availabilityStatus === "CONFIRMED"; return <div className="programmingRosterRow" key={`${item.queueCycleId}-${item.batchOrder}`}><div className="programmingOrder"><span>Batch</span><strong>#{item.batchOrder}</strong></div><div className="programmingTruck"><strong>{registration}</strong><span>{driverName}</span></div><div><StatusBadge value={item.availabilityStatus} /></div><label className="atcField"><span>Order and ATC</span><select disabled={!eligible || busy !== null} onChange={(event) => setOrderByCycle((current) => ({ ...current, [item.queueCycleId]: event.target.value }))} value={orderByCycle[item.queueCycleId] ?? ""}><option value="">Select imported order</option>{orders.map((order) => <option disabled={Object.entries(orderByCycle).some(([cycle, orderId]) => cycle !== item.queueCycleId && orderId === order.orderId)} key={order.orderId} value={order.orderId}>{order.atcNo} · SO {order.salesOrderNo} · {order.dprpCustomerName ?? order.customerName}</option>)}</select></label></div>; })}</div> : null}{liveItems.length ? <div className="confirmBar"><p>{allOrdersSelected ? "Ready to program confirmed trucks in FIFO order." : "Choose an imported order and ATC for each confirmed truck, or let the system pair them in queue order."}</p><button className="secondaryButton commandButton" disabled={busy !== null || confirmed.length === 0} onClick={autoAssignOrders} type="button"><Layers3 size={16} />Auto-assign ATCs</button><button className="primaryButton commandButton" disabled={busy !== null || !allOrdersSelected} onClick={() => void programConfirmed()} type="button">{busy === "program" ? <LoaderCircle className="spin" size={16} /> : <CheckCircle2 size={16} />}Confirm programming</button></div> : null}</> : null}
