...
import { reportFleetReturn, requestFleetBypass, subscribeToIssuedBypassCodes, subscribeToMyBypassRequests, validateFleetBypassOtp, type IssuedBypassCode, type MyBypassRequest } from "./api";
...
export function FleetWorkspace({
  ...
  const [myBypasses, setMyBypasses] = useState<MyBypassRequest[]>([]);

  useEffect(() => subscribeToIssuedBypassCodes(siteId, fleetOfficerId, setIssuedCodes), [fleetOfficerId, siteId]);
  useEffect(() => subscribeToMyBypassRequests(siteId, fleetOfficerId, setMyBypasses), [fleetOfficerId, siteId]);
  ...
  return <>
    ...
    <section className="fleetActionBoard">
      ...
      <div className="fleetActionSet"><button className="secondaryButton commandButton" disabled={bypassEligibleQueue.length === 0} onClick={() => open("bypass")} type="button"><ListOrdered size={16} />Request bypass</button><button className="primaryButton commandButton" disabled={returnableTrucks.length === 0} onClick={() => open("return")} type="button"><Truck size={16} />Report return</button></div>
    </section>
    <div className="fleetWorkbench">
      ...
      {issuedCodes.length ? null : <p className="fleetOtpHint">An approved bypass code appears here automatically. Nothing to type.</p>}
      {myBypasses.length ? <div className="bypassStatusList">{myBypasses.map((item) => { const truck = myTrucks.find((entry) => entry.id === item.truckId); return <div className="bypassStatusRow" key={item.id}><div><strong>{truck?.registrationNumber ?? "Truck"}</strong><small>Requested {item.requestedAt}</small></div><div><StatusBadge value={item.status} />{item.rejectionReason ? <small className="bypassRejection">{item.rejectionReason}</small> : null}</div></div>; })}</div> : null}</section>
    </div>
