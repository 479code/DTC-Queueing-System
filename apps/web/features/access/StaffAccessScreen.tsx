...
import { KeyRound, Mail, Pencil, Plus, Search, ShieldCheck, UserRoundCheck, UsersRound } from "lucide-react";
...
type FormState = { name: string; email: string; roles: UserRole[]; isActive: boolean; mfaRequired: boolean };
const emptyForm: FormState = { name: "", email: "", roles: ["fleetOfficer"], isActive: true, mfaRequired: false };
...
export function StaffAccessScreen({ siteId, currentUserId }: Props) {
  ...
  const openEdit = (user: StaffAccessView) => { setForm({ name: user.name, email: user.email, roles: user.roles, isActive: user.isActive, mfaRequired: user.mfaRequired }); setMessage(""); setEditing(user); };
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      if (editing) {
        ...
        await provisionStaffAccess(siteId, { name: form.name, email: form.email, roles: form.roles, mfaRequired: form.mfaRequired });
