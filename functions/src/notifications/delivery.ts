...
type RegisteredDevice = { id: string; fcmToken: string };

export async function deliverNotificationForRecord(siteId: string, notificationId: string): Promise<void> {
  const notificationRef = notificationsRef(siteId).doc(notificationId);
  const snapshot = await notificationRef.get();
  if (!snapshot.exists) return;

  const claimResult = await db.runTransaction(async (transaction) => {
    const latestSnapshot = await transaction.get(notificationRef);
    const latest = latestSnapshot.data() as
      | (PushNotificationSource & { userId?: string; deliveryStatus?: string; deliveryClaimedAt?: Timestamp })
      | undefined;
    if (!latest) return null;
    if (["SENT", "PARTIAL", "FAILED", "NO_DEVICES"].includes(latest.deliveryStatus ?? "")) return null;
    const leaseIsFresh = latest.deliveryStatus === "SENDING" && latest.deliveryClaimedAt instanceof Timestamp && latest.deliveryClaimedAt.toMillis() > Date.now() - 5 * 60 * 1000;
  ...
  if (claimResult === "BUSY" || !claimResult) return;
  ...
  const deviceSnapshot = await userDevicesRef(siteId, source.userId).where("isActive", "==", true).get();
  ...
  batch.update(notificationRef, {
    deliveryStatus: successCount === 0 ? "FAILED" : failureCount > 0 ? "PARTIAL" : "SENT",
