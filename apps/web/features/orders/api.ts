...
const demoImports: OrderImportView[] = [{ id: "orders-demo-160926", originalFileName: "DD SUS OIL 16.09.26.xlsx", status: "PROCESSED", rowsProcessed: 2, uploadedAt: "16 Sep, 09:10" }];
...
function mapImport(document: QueryDocumentSnapshot<DocumentData>): OrderImportView {
  ...
  return { id: document.id, originalFileName: String(data.originalFileName ?? document.id), status: String(data.status ?? "FAILED"), rowsProcessed: Number(data.rowsProcessed ?? 0), uploadedAt: dateText(data.uploadedAt) };
}
...
export async function uploadOrders(siteId: string, file: File, demoMode: boolean): Promise<{ importId: string; rowsProcessed: number }> {
  ...
  if (demoMode || !auth?.currentUser) return { importId: "orders-demo-160926", rowsProcessed: 2 };
  const importId = crypto.randomUUID().replaceAll("-", "");
  const digest = await crypto.subtle.digest("SHA-256", await file.arrayBuffer());
  const checksum = Array.from(new Uint8Array(digest), (byte) => byte.toString(16).padStart(2, "0")).join("");
  if (usesRailwayOperations()) return uploadOrderWorkbook<{ rowsProcessed: number }>({ siteId, importId, file, checksum }).then((result) => ({ importId: result.importId, rowsProcessed: result.summary.rowsProcessed }));
  ...
  const processed = await httpsCallable<{ siteId: string; importId: string }, { rowsProcessed: number }>(functions, "processOrderImport")({ siteId, importId });
  return { importId, rowsProcessed: processed.data.rowsProcessed };
}
